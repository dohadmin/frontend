...
import ViewTraineeRequestSchema from '../../../schemas/ViewTraineeRequestSchema.js'
import Button from '../../ui/Button'
import ImageInput from '../../ui/ImageInput'
import axios from 'axios'
import useAccountStore from '../../../stores/trainer/AccountStore.js';

const UpdateTraineeForm = ({setOpen, trainee, setData}) => {
  ...
  const { control, handleSubmit, formState: { errors }, reset, setError} = useForm({
    ...
    defaultValues:{
      ...
      civilStatus: trainee.civilStatus,
      ...
      controlNumber: trainee.controlNumber,
      phoneNumber: trainee.phoneNumber,
      email: trainee.email,
      trainerId: trainee.trainerId._id,
    }
  });
  ...
  return (
    <form className="p-6 flex flex-col items-start gap-4 " onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className=" flex flex-col gap-2 w-full">        
        <label className="text-sm uppercase text-neutral-700 font-medium">Control Number</label>
        <Controller
          name="controlNumber"
          control={control}
          render={({ field: { value, onChange } }) => (
            <Input
              className="w-full"D
              value={value}
              placeholder='553491-185437-49542-635423'
              onChange={(e) => onChange(e.target.value)}
              didError={!!errors.controlNumber}         
            />
          )}
        />
        <label className="text-sm text-gray-500 ">A unique number that was provided by the agency to the
          <span className="text-amber-500 mx-1 font-medium">Trainee</span>
        </label>
      </div>
      <div className=" flex flex-col gap-2 w-full">        

    ...
    <button 
      className={className + ` 
        ...
        ${didError  && outline === true ? "ring-red-500 outline outline-offset-1 outline-3 outline-rose-100" : 
          (isDisabled ? "ring-gray-200 cursor-not-allowed text-gray-500 ": (checked ? "ring-amber-500" : "ring-gray-200"))} flex items-center justify-start rounded-md `} 
      onClick={handleClick}
      disabled={isDisabled}
      type="button"
    >
      ...
      />
      <label 
        className={`text-sm  ${didError ? "text-rose-500 font-normal":( isDisabled ? "text-gray-500 cursor-not-allowed" : ( checked ? "  text-gray-700 font-normal" :"text-gray-500 font-normal cursor-pointer"))}`}
      >
        {value}
      </label>
    </button>

...
const ProfileBranch = ({setActiveBranch, setData}) => {


  const selectedTrainee = useAccountStore(state => state.user);
  ...
  const { data: trainings, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await axios.get(`https://server-np0x.onrender.com/training/get-trainings-by-trainee/${selectedTrainee._id}`);
